...
int CheckFai_and_ReturnMaxChrSize(string faiFileName)
{
	...
	int maxContigSizeInFai = 0;
        ...
        if(!finFai.is_open())
        {
                ...
                cout << "Please run 'samtools faidx " << faiFileName.substr(0,faiFileName.length()-3) << "'";
		exit(99);
        }
...
void CreateDirectoryIfDoesNotExist(string name)
{
	...
	struct stat st = {0};
	if(stat(temporaryDirectoryName.c_str(), &st) == -1)
	{
		mkdir(temporaryDirectoryName.c_str(), 0700);
	}
}
...
void ConfigureRelatedParameters(string coraExecPath)
{
	...
	if(RepresentativeMappingMode == "MRSFAST")
	{
		noHitMode = "FA";
	}
	else //"MRSFAST_ULTRA" || RepresentativeMappingMode ==  "BOWTIE" || RepresentativeMappingMode == "BWA" || RepresentativeMappingMode == "BOWTIE_2" || "BWA_MEM"
	{
		noHitMode = "SAM";
	}
...
void PrintHomManual(string error)
{
	if(error != "")
	{
		...
		cout << "Please see manual below... " << endl << endl;
	}
	...
	cout << "Notes: Reference should be in fasta or multi-fasta format and indexed using samtools faidx" << endl;
	cout << "       ExactHom and InexactHom are arbitrary file names to be used in the search pipeline of CoRA" << endl << endl;

	cout << "Important Options   : -K [INT]       k-mer length [no default]" << endl;
	cout << "                    : -H [INT] Hamming-distance threshold per k-mer for inexact homology table. (1 to 3) [default 2] " << endl << endl;

	cout << "Performance Options : -p [INT] Number of physical file splits for computing the homology tables (1 to 10) [default 10]" << endl;
	cout << "                               Higher values substantially reduce memory use, sacrificing some run time." << endl;
	cout << "                      -t [INT] Number of parallel threads to use for computing the inexact homology table (1 to 24) [default 8]" << endl << endl;
	exit(0);
}
...
void PrintReadFileGenManual(string error)
{
	...
	cout << "======================================================================================" << endl;
	cout << "Usage: To generate single-end read file list: " << endl;
	cout << "          cora " << READFILEGEN << " [options] <FileListName> -S <FASTQ-A> <FASTQ-B> <...>" << endl;
	cout << "       To generate paired-end read file list: " << endl;
	cout << "          cora " << READFILEGEN << " [options] <FileListName>  -P <FASTQ-A-first> <FASTQ-A-second> <FASTQ-B-first> <FASTQ-B-second> <...>" << endl;
	cout << "======================================================================================" << endl << endl;
	cout << "Notes: <FileListName> is the output file list for CoRA" << endl;
	cout << "       FASTQ files are input read datasets. In the current version paired and single-end read datasets cannot be mixed" << endl;
	cout << "       This restriction will be lifted in future updates" << endl << endl;
...
void PrintMapperIndexManual(string error)
{
	...
	cout << "Notes: Reference should be in fasta or multi-fasta format and indexed using samtools faidx" << endl << endl;

	cout << "Important Options   : --Map    off-the shelf mapper to be used for coarse mapping (used in stage 2 and 3) [default BWA]" << endl;
	cout << "                               current valid options: BWA, BWA_MEM, BOWTIE, BOWTIE_2, MRSFAST, MRSFAST_ULTRA" << endl;
	cout << "                    : --Exec   name of the executable file if not installed or installed by name different than the default [default bwa]" << endl;
	cout << "                               Either full path (e.g. /home/bwa_folder/bwa) or just executable name if installed (e.g. bwa)" << endl;
	cout << "                    : --Index  full path for the index to be constructed if [default is same as <Reference>]" << endl;
	cout << endl;
	
	cout << "Performance Options : --opt    Additional options to be passed to the mapper for indexing" << endl;
...
void PrintSearchManual(string error)
{
	...
	cout << "       <Read_File_List> can be generated using " << READFILEGEN << " command or manually (see README)" << endl << endl;

	cout << "Important Options   : -C          Determines which stages of CORA's compressive search pipeline will be executed. [default 1111]" << endl;
	...
	cout << "                                  4) Traverse links and homology tables simultaneously for generating final mappings." << endl;

	cout << "                    : --Mode      Mapping report mode: ALL, BEST, BEST_SENSITIVE, BEST_FAST, STRATUM, or UNIQUE [default ALL]." << endl;

	cout << "                    : --Map       Off-the shelf mapper to be used for coarse mapping (used in stage 2 and 3) [default BWA]" << endl;
	cout << "                                  Current valid options: BWA, BWA_MEM, BOWTIE, BOWTIE_2, MRSFAST, MRSFAST_ULTRA, MANUAL" << endl;

	cout << "                    : --Exec      Name of the executable file to be run for coarse mapping" << endl;
	cout << "                                  Required for Stage 2, either full path (e.g. /home/bwa_folder/bwa) or executable name if installed (e.g. bwa)" << endl;

	cout << "                    : -R          Read input mode, SINGLE or PAIRED [default PAIRED]" << endl;

	cout << "                    : --MinI      [INT] for PAIRED mode, minimum read fragment insert length (|tLen| in SAM format) [default 150]" << endl;
	cout << "                    : --MaxI      [INT] for PAIRED mode, maximum read fragment insert length (|tLen| in SAM format) [default 650]" << endl;

	cout << "                    : -O          Output SAM file to print the final mapping output [default = CoRA_Output.sam]" << endl;

	cout << "                    : -K          k-mer compression mode, HALF or FULL [HALF]." << endl;

	cout << "                    : -L [INT]    Read length for CoRA mapping [no default]. " << endl;
	cout << "                                  Currently if -K is HALF, -L needs to be an even number. This restriction will be removed in the next update" << endl;

	cout << "                    : --Metric    Distance metric to be used for mapping (HAMMING or EDIT) [default HAMMING]" << endl;
	cout << "                                  HAMMING distance is substitution only. EDIT (Levenshtein) distance allows indels (currently only compatible with BWA and BOWTIE_2 coarse mappers)" << endl;
	
	cout << "                    : -E [INT]    Hamming/Edit-distance threshold per k-mer for inexact homology table. (1 to 3) [default 2] " << endl;
	cout << "                                  Use of Hamming or Edit distance is determined by --Metric option." << endl;
	cout << "                                  In the current version this value should be the same as the one used to generated the homology table." << endl;
	cout << "                                  This restriction will be lifted in future updates." << endl << endl;

	cout << "                    : --PrintR    Include the original read names in the final SAM output file. (0 or 1) [default 0]" << endl;
	cout << "                    : --PrintQ    Include quality scores and original read names in the final SAM output file. (0 or 1) [default 0]" << endl;
	cout << "                                  PrintQ = 1 overrides PrintR = 0" << endl << endl;

	cout << "Performance Options : --memoi     [INT] Memoization (in stage 4) for number of k-mers appearing at least as many times as this value [default 20]." << endl;
	cout << "                                  Must be larger than 1. Lower values save runtime during Stage 4 using more memory." << endl;
	
	cout << "                      --fs        [INT or STRING] Number of physical file splits for Input FASTQ file for compression (stage 1)" << endl;
	...
	cout << "                                  Higher values reduce memory use during Stage 1 sacrificing some run time. " << endl;
        cout << "                                  AUTO mode automatically determines the number of FASTQ splits as follows: 1 + genome_size / 10M (which corresponds to --fs 32 for human genome)" << endl << endl;	

	cout << "                      --cm        [STRING] Mode for Stage 1 determining whether the reference should be used for compressing reads or not." << endl;
	cout << "                                  Options: WITHREF or NOREF [default WITHREF]" << endl << endl;

	cout << "Paralelization Opt. : --coarseP   [INT] The number of parallel threads to be used for coarse mapping. Capped at maximum number of threads allowed by coarse-mapper [default is 1]" << endl << endl;

	cout << "Misc. Options       : --TempDir   [STRING] The directory to be used for intermediate CORA files. [default __temporary_CORA_files]." << endl;
	cout << "                                  This enables running two CORA jobs in the same folder as long as their temporary directories are different." << endl;
	cout << "                                  If you are running only part of the CORA pipeline (e.g. 0001 or 0111), make sure the intermediate files from previous" << endl;
	cout << "                                  stages exist in the temporary directory." << endl << endl; 
...
void ParseCommandLineArguments(int argc, char* argv[])
{
	...
	if(argc == 1 || string(argv[1]) == "-v" || string(argv[1]) == "--help" || string(argv[1]) == "help")
	{
		...
		cout << endl << "To print the manual for each command, run command without any options." << endl << endl;
	...
	else if(firstArg == SEARCH)
	{
		...
		else
		{
			for(int i=2; i<argc; )
			{
				...
				else if(argMark == "--PrintR")
				{
					PrintRFlag = atoi(argv[i+1]);
					i+=2;
				}
				else if(argMark == "--PrintQ")
				{
					PrintQFlag = atoi(argv[i+1]);
					i+=2;
				}
				else if(argMark == "-R")
				{
			...
			if(PrintQFlag == 1) //PrintQ=1 overrides PrintR=0
			{
				PrintRFlag = 1;
			}
	
			if(mappingReportMode == "BEST_FAST" && mapperMetric != "EDIT")
			{
				PrintSearchManual("BEST_FAST mapping mode is only available in the EDIT metric");	
			}
			...
			if(splitMode == "HALF")
			{
				if(readLen % 2 == 1)
				{
					PrintSearchManual("Currently CoRA pipeline requires -L to be even when -K is set to HALF. This restriction will be lifted in a future update.");
				}	
			}
			else if(splitMode == "THREEWAY")
			{
				if(readLen % 3 != 0)
				{
					PrintSearchManual("Currently CoRA pipeline requires -L to be divisible by three when -K is set to THREEWAY. This restriction will be lifted in a future update.");
				}
			}
	...
	{
		cout << "The first argument should be '" << HOMINDEX << "', '" << READFILEGEN << "', '" << MAPPERINDEX << "' , or '" << SEARCH << "'. For help, run \"cora help\"" << endl << endl;
		exit(98);
	}
}
...
int main(int argc, char* argv[])
{
	...
	string msMapNoHitFile = msMapFile + ".nohit";
	...
	if(runMode[0] >= '1')
	{
		...
		if(numFastqSplits == 1)
		{
			...
			CollapseCall << " " << refLineLen << " " << readLen << " " << perfMapsFile << " " << maxChrSizeInRef << " " << collapsedFastqFile_longReadsList << " " << inputMode << " " << splitMode << " " << RepresentativeMappingMode;
		...
		else
		{
			...
			while(fragFile >> fragLine)
			{
				...
				fragmentedCollapseCall << " " << refLineLen << " " << readLen << " " << fragmentedPerfMapsFileSS.str() << " " << maxChrSizeInRef << " " << fragmentedLongReadsListSS.str() << " " << inputMode << " " << splitMode << " " << RepresentativeMappingMode << " " << fragLine << " " << fragSignalSize << " " << killSignalFile << " " << numLRNs;
	...
	if(runMode[1] >= '1')
	{
		...
		else if(RepresentativeMappingMode == "MRSFAST") //This is the best-mapping version of original mrsFAST (earlier than 3.0)
		{
			...
			else
			{
				...
				if(subErrorThreshold == 2)
				{
					...
					stringstream MoveNoHitFileCall; //Only E2 nohits are important
					MoveNoHitFileCall << "mv " << msMapFile_E2 << ".nohit " << msMapFile << ".nohit";
					ExecuteSystemCall(MoveNoHitFileCall, VERIFY);
				}
				else if(subErrorThreshold == 3)
				{
					...
					stringstream MoveNoHitFileCall; //Only E2 nohits are important
					MoveNoHitFileCall << "mv " << msMapFile_E3 << ".nohit " << msMapFile << ".nohit";
					ExecuteSystemCall(MoveNoHitFileCall, VERIFY);
				}
				else
				{
		...
		//[TODO] Convert awk into faster custom extraction
		if(RepresentativeMappingMode == "MRSFAST_ULTRA" || RepresentativeMappingMode ==  "BOWTIE" || RepresentativeMappingMode == "BWA" || RepresentativeMappingMode == "BOWTIE_2" || RepresentativeMappingMode == "BWA_MEM") //Mrsfast-ultra prints nohits within the mapping output, so this extracts them out
		{
			stringstream noHitFileGenerationCall;
			//This is for creating a plain sam format nohit file
			noHitFileGenerationCall << "LC_ALL=C awk '$2 == 4' " << msMapFile << " > " << msMapNoHitFile << endl;
			ExecuteSystemCall(noHitFileGenerationCall, VERIFY);
		}
	}
	...
	if(runMode[3] >= '1')
	{
		...
		mapAllInferCall << homTablePerfectMatchFile << " " << homTableInexactMatchFile << " " << readLen << " " << refFile << " " << refLineLen << " " << pibLinks << " ";
		...
		mapAllInferCall << mappingReportMode << " NULL " << globalMapCountLimit << " " << mappingOutputFile << " UNCOLLAPSED NULL NULL " << (int) idDigitLen << " " << splitMode << " " << inputMode << " ";
		...
		mapAllInferCall << msMapNoHitFile << " " << collapsedFastqFile_longReadsList << " " << noHitMode << " " << numTotalReads << " " << memoizationThreshold << " " << numMismatchesPerReadMer << " " << numMismatchesPerReadMer << " ";

		//PrintR and PrintQ related params
		mapAllInferCall << PrintRFlag << " " << PrintQFlag << " " << fastqInputListFile;
