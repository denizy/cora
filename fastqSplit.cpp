...
unsigned char GetNuc(unsigned char c) //Used only for setup, no need to optimize
{
    switch(c)
    {
        case 0:
...
int main(int argc, char* argv[])
{
    ...
    if(fragSignalSize == 2)
    {
        ...
        else if(mappingModeStr == "SPLIT_SINGLE")
        {
            ...
            unsigned short halfReadLength = readLength / 2; //If full read length is odd, ignore the last base at this stage
            ...
            for(int k=0; k<numSamples; k++)
            {
                ...
                while(true)
                {
                    ...
                    groupNo = GetFragGroupNo_2(readLine[halfReadLength], readLine[2*halfReadLength-1]);
        ...
        else if(mappingModeStr == "THREEWAY_SINGLE")
        {
            ...
            for(int k=0; k<numSamples; k++)
            {
                ...
                while(true)
                {
                    ...
                    //this is second split
                    groupNo = GetFragGroupNo_2(readLine[oneThirdReadLength], readLine[2*oneThirdReadLength-1]);
                    AddToBuffer(groupNo, readCode, readLine + oneThirdReadLength, oneThirdReadLength);
                    readCode += 2; //switching to the first split of the next read

                    //this is third split
                    groupNo = GetFragGroupNo_2(readLine[2 * oneThirdReadLength], readLine[3*oneThirdReadLength-1]);
        ...
        else if(mappingModeStr == "SPLIT_PAIRED")
        {
            char *readLine1, *readLine2;
            unsigned long long readCode = 0;
            unsigned short halfReadLength = readLength / 2; //If the full read length is odd, ignore the last base at this stage
            ...
            for(int k=0; k<numSamples; k++)
            {
                ...
                while(true)
                {
                    ...
                    groupNo = GetFragGroupNo_2(readLine1[halfReadLength], readLine1[2*halfReadLength-1]);
                    ...
                    groupNo = GetFragGroupNo_2(readLine2[halfReadLength], readLine2[2*halfReadLength-1]);
        ...
        else if(mappingModeStr == "THREEWAY_PAIRED")
        {
            ...
            for(int k=0; k<numSamples; k++)
            {
                ...
                while(true)
                {
                    ...
                    groupNo = GetFragGroupNo_2(readLine1[oneThirdReadLength], readLine1[2*oneThirdReadLength-1]);
                    ...
                    groupNo = GetFragGroupNo_2(readLine1[2*oneThirdReadLength], readLine1[3*oneThirdReadLength-1]);
                    AddToBuffer(groupNo, readCode, readLine1 + oneThirdReadLength, oneThirdReadLength);
                    readCode += 2; //switching to the first split of the second mate //for direction jumps (they're all forward at this point)
                    ...
                    groupNo = GetFragGroupNo_2(readLine2[oneThirdReadLength], readLine2[2*oneThirdReadLength-1]);
                    ...
                    groupNo = GetFragGroupNo_2(readLine2[2 * oneThirdReadLength], readLine2[3*oneThirdReadLength-1]);
    ...
    else if(fragSignalSize == 4)
    {
        ...
        else if(mappingModeStr == "SPLIT_SINGLE")
        {
            ...
            for(int k=0; k<numSamples; k++)
            {
                ...
                while(true)
                {
                    ...
                    //this is second split
                    groupNo = GetFragGroupNo_4(readLine[halfReadLength], readLine[halfReadLength+1], readLine[2*halfReadLength-2], readLine[2*halfReadLength-1]);
                    AddToBuffer(groupNo, readCode, readLine + halfReadLength, halfReadLength);
                    readCode += 2; //switching to the first split of the next read
                }   
                fclose(finRead);
        ...
        else if(mappingModeStr == "THREEWAY_SINGLE")
        {
            ...
            for(int k=0; k<numSamples; k++)
            {
                ...
                while(true)
                {
                    ...
                    //this is second split
                    groupNo = GetFragGroupNo_4(readLine[oneThirdReadLength], readLine[oneThirdReadLength+1], readLine[2*oneThirdReadLength-2], readLine[2*oneThirdReadLength-1]);
                    AddToBuffer(groupNo, readCode, readLine + oneThirdReadLength, oneThirdReadLength);
                    readCode += 2; //switching to the first split of the next read
                    
                    //this is third split
                    groupNo = GetFragGroupNo_4(readLine[2*oneThirdReadLength], readLine[2*oneThirdReadLength+1], readLine[3*oneThirdReadLength-2], readLine[3*oneThirdReadLength-1]);
                    AddToBuffer(groupNo, readCode, readLine + 2 * oneThirdReadLength, oneThirdReadLength);
                    readCode += 2; //switching to the first split of the next read
                }   
                fclose(finRead);
        ...
        else if(mappingModeStr == "SPLIT_PAIRED") ////If the full read length is odd, ignore the last base at this stage
        {
            char *readLine1, *readLine2;
            unsigned long long readCode = 0;
            unsigned short halfReadLength = readLength / 2; //If the full read length is odd, ignore the last base at this stage
            ...
            for(int k=0; k<numSamples; k++)
            {
                ...
                while(true)
                {
                    ...
                    groupNo = GetFragGroupNo_4(readLine1[halfReadLength], readLine1[halfReadLength+1], readLine1[2*halfReadLength-2], readLine1[2*halfReadLength-1]);
                    AddToBuffer(groupNo, readCode, readLine1 + halfReadLength, halfReadLength);
                    readCode += 2; //switching to the first split of the second mate //for direction jumps (they're all forward at this point)
                    ...
                    groupNo = GetFragGroupNo_4(readLine2[halfReadLength], readLine2[halfReadLength+1], readLine2[2*halfReadLength-2], readLine2[2*halfReadLength-1]);
                    AddToBuffer(groupNo, readCode, readLine2 + halfReadLength, halfReadLength);
                    readCode +=2 ; //switching to the first split of the first mate of the next read
                }
        ...
        else if(mappingModeStr == "THREEWAY_PAIRED")
        {
            ...
            for(int k=0; k<numSamples; k++)
            {
                ...
                while(true)
                {
                    ...
                    groupNo = GetFragGroupNo_4(readLine1[oneThirdReadLength], readLine1[oneThirdReadLength+1], readLine1[2*oneThirdReadLength-2], readLine1[2*oneThirdReadLength-1]);
                    AddToBuffer(groupNo, readCode, readLine1 + oneThirdReadLength, oneThirdReadLength);
                    readCode += 2; //switching to the third split of the first mate //for direction jumps (they're all forward at this point)

                    groupNo = GetFragGroupNo_4(readLine1[2*oneThirdReadLength], readLine1[2*oneThirdReadLength+1], readLine1[3*oneThirdReadLength-2], readLine1[3*oneThirdReadLength-1]);
                    ...
                    groupNo = GetFragGroupNo_4(readLine2[oneThirdReadLength], readLine2[oneThirdReadLength+1], readLine2[2*oneThirdReadLength-2], readLine2[2*oneThirdReadLength-1]);
                    AddToBuffer(groupNo, readCode, readLine2 + oneThirdReadLength, oneThirdReadLength);
                    readCode +=2 ; //switching to the third split of the second mate
                    
                    groupNo = GetFragGroupNo_4(readLine2[2*oneThirdReadLength], readLine2[2*oneThirdReadLength+1], readLine2[3*oneThirdReadLength-2], readLine2[3*oneThirdReadLength-1]);
                    AddToBuffer(groupNo, readCode, readLine2 + 2 * oneThirdReadLength, oneThirdReadLength);
                    readCode +=2 ; //switching to the first split of the first mate of the next read
                }
